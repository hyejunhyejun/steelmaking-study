...
def test_unified_groups_share_wording():
    """겹치는 문항은 회차·유형 어디서 보든 같은 문장이어야 외우기 쉽다."""
    d = build(ROUNDS, PHOTOS, XLSX, DATA)
    by = {q["qid"]: q for c in d["rounds"] + d["topics"] for q in c["questions"]}
    same = [
        ("t01-1", "25-2-13"),            # 열풍로 내연·외연
        ("21-1-14", "23-2-3"), ("21-1-14", "24-2-18"),
        ("t01-7", "25-2-20"),            # 풍구 파손
        ("t43-150", "22-1-8"),           # 코크스 강도시험
        ("t43-149", "21-2-20"),          # 코크스 품질시험
        ("t30-117", "24-2-15"),          # 소결광 제조과정
        ("t35-131", "24-1-7"),           # 노열 과다 대책
        ("t35-132", "24-1-13"),          # 부착물
        ("t21-88", "24-2-10"),           # 측면 냉각방식
        ("t29-112", "23-2-16"),          # 소결 통기도
        ("t03-19", "21-2-3"),            # 탄화시간
        ("t18-80", "23-1-15"),           # 환원 순서
        ("t14-65", "22-1-19"),           # DL 소결기 설비
        ("t16-73", "23-2-8"), ("t16-73", "25-1-9"),   # 뱅킹
        ("t34-126", "21-1-12"),          # 5대 원소
        ("t11-53", "25-1-5"),            # 노정장입 구비조건
        ("22-1-9", "24-1-3"),            # 노정장입 제어장치
    ]
    # 소제목은 문항마다 묻는 표현이 달라 다를 수 있으니 답만 비교한다
    def answers(qid):
        return [p["answers"] for p in by[qid]["parts"]]
    for a, b in same:
        assert answers(a) == answers(b), (a, b)
    # 개수만 다른 묶음: 적은 쪽이 많은 쪽의 앞부분과 일치
    for few, many in [("t09-45", "24-1-4"), ("24-2-13", "t06-35"),
                      ("t03-21", "24-1-14")]:
